...
async fn quote_jupiter(http: &reqwest::Client, params: &SellParams) -> anyhow::Result<Quote> {
    let resp: serde_json::Value = http
        .get(JUPITER_QUOTE_URL)
        .query(&[
            ("inputMint", params.input_mint.as_str()),
            ("outputMint", SOL_MINT),
            ("amount", &params.amount_tokens.to_string()),
...
async fn quote_kamino(http: &reqwest::Client, params: &SellParams) -> anyhow::Result<Quote> {
    let resp: serde_json::Value = http
        .get(KAMINO_SWAP_URL)
        .query(&[
            ("tokenIn", params.input_mint.as_str()),
            ("tokenOut", SOL_MINT),
            ("amountIn", &params.amount_tokens.to_string()),
            ("maxSlippageBps", &params.slippage_bps.to_string()),
            ("wallet", params.user_public_key.as_str()),
...
pub async fn build_sell_transaction(params: SellParams) -> Result<BuildTxResult, String> {
    ...
    let best = quotes
        ...
        .ok_or_else(|| "No sell route found (Jupiter and Kamino both failed)".to_string())?;
    ...
    let out_amount_ui = best.out_amount as f64 / 1e9;
